...
if(get_post_meta($post->ID, 'cpt_pdf_attachment'.$i, true)){
	echo '<a href="'.get_post_meta($post->ID, 'cpt_pdf_attachment'.$i, true).'">'.__('Download', 'custom-post-type-pdf-attachment').'</a> ';
}
if(get_post_meta($post->ID, 'cpt_pdf_attachment'.$i, true)){
	echo __('Check to remove','custom-post-type-pdf-attachment').' '.form_class::form_checkbox('cpt_pdf_attachment_remove'.$i,'cpt_pdf_attachment_remove'.$i,$i,'','','','','','',false);
}
?>
</td>
</tr>
